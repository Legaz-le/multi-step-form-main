...
const Step = ({ number, label, desc, isFirst }: StepType) => {
  ...
  return (
    <div className={`flex  items-center  md:space-x-4 ${!isFirst ? "md:pt-5 " : ""}`}>
      <div
      ...
      <div>
        <span className="text-Grey-500 text-[13px] font-[40] hidden md:block">{label}</span>
        <p className="text-White text-[13px] font-[500] tracking-widest hidden md:block">
          {desc}
        </p>
      </div>
    </div>
